...
description: Tento dokument popisuje postup použití volaných pro nepřetržitou integraci s aplikacemi Xamarin. Popisuje, jak instalovat, konfigurovat a používat volaných.
ms.prod: xamarin
...
Po dokončení instalačního programu Jenkins.App bude spustit volaných a spustit webový prohlížeč s adresou URL http://localhost:8080, jak ukazuje následující snímek obrazovky:
...
Automatizované testy můžete odeslat testovací cloudu pomocí příkazů prostředí. Další informace o nastavení testu spusťte v Xamarin Test Cloud v této příručce pro použití [Xamarin.UITest](/appcenter/test-cloud/preparing-for-upload/uitest/).
