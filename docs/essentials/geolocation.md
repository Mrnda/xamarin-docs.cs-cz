...
[ `Location` ](xref:Xamarin.Essentials.Location) a [ `LocationExtensions` ](xref:Xamarin.Essentials.LocationExtensions) třídy definují `CalculateDistance` metody, které vám umožní vypočítat vzdálenost mezi dvěma zeměpisné umístění. To vypočítat vzdálenost nevyžaduje cesty nebo jiné cesty v úvahu a jenom nejkratší vzdáleností mezi dvěma body společně na povrch zemském povrchu, je také označován jako _dobře kroužek vzdálenost_ nebo colloquially, vzdálenost "jako dovézt."
...
`Location` Konstruktor má zeměpisnou šířku a délku argumenty v tomto pořadí. Kladné hodnoty zeměpisné šířky jsou severně od rovníku a hodnoty zeměpisné délky kladné východ od základního poledníku. Použít konečný argument `CalculateDistance` určete miles nebo kilometrech. `Location` Třída také definuje `KilometersToMiles` a `MilesToKilometers` metod pro převod mezi dvě jednotky.
